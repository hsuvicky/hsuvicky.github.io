...
for(const file of ['index.html','japandi.css','japandi.js','styles.css','script.js','resume.html','assets','about','hobbies','.nojekyll'])fs.cpSync(path.join(root,file),path.join(dist,file),{recursive:true});
const html=fs.readFileSync(path.join(root,'index.html'),'utf8');
for(const m of html.matchAll(/(?:src|href)="(\/[^"#]*)"/g)){if(!fs.existsSync(path.join(dist,m[1])))throw new Error('Missing asset: '+m[1]);}
